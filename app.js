...
fastify.post("/contact/submit", (request, reply) => {
  ...
  const html = [
    ...
    "<br />",
    `<p><strong>Sent At:</strong> ${(/* @__PURE__ */ new Date()).toLocaleTimeString([], { timeZone: "America/New_York" })}, ${(/* @__PURE__ */ new Date()).toLocaleDateString([], { timeZone: "America/New_York" })} (EST)</p>`,
    "</div>",
    "</div>"
  ].join("");
  const mailOptions = {
